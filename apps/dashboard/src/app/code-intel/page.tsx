  ...
  return (
    <div className="space-y-6">
      ...
      <form
        ...
        className="space-y-4 rounded-lg border border-white/[0.06] bg-neutral-950 p-4"
      >
        <div className="grid gap-3 md:grid-cols-[minmax(0,1.4fr)_minmax(220px,1fr)_auto]">
          <input
            ...
            className="rounded-lg border border-white/[0.06] bg-neutral-950 px-4 py-2 text-sm text-white placeholder-neutral-600 focus:border-blue-500/50 focus:outline-none"
          />
          <input
            type="text"
            value={repoName}
          ...
          <button
            ...
            className="rounded-lg border border-white/[0.06] bg-black px-4 py-2 text-sm font-medium text-white transition-colors hover:border-white/[0.1] disabled:cursor-not-allowed disabled:opacity-60"
          >
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {repositories.map((repo: Repository) => (
            <div
              key={repo.id}
              className="relative rounded-lg border border-white/[0.06] bg-neutral-950 p-4"
            >
              <button
                type="button"
              ...
              <Link href={`/code-intel/${repo.id}`} className="block pr-16">
                <div className="flex items-start justify-between gap-3">
                  <p className="font-medium text-neutral-200">{repo.name}</p>
                  <StatusBadge status={repo.status} />
                </div>

                <p className="mt-2 truncate text-xs text-neutral-500">{repo.url}</p>
